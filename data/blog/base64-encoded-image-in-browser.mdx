...
- 두 가지 부분집합인 URL과 URN으로 구성됨
  - **URN(Uniform Resource Name): 리소스 이름**
  
  리소스가 어디에 있는지와 상관없이 그 이름만으로 리소스를 식별

  - **URL(Uniform Resource Locator): 리소스 위치**
   
  리소스가 어디에 있는지를 설명해서 리소스를 식별
...
- 브라우저가 정보를 찾는데 필요한 *리소스*의 위치

  cf. 리소스: 웹에서 사용되고 있으며 식별할 수 있는 모든 자원
...
- <u>리소스를 가져오려면 어떤 프로토콜을 사용하여 서버에 접근해야 하는지</u>를 알려준다.
- 예시
